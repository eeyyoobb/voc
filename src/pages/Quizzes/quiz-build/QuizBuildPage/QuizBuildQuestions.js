import React, { useState, useEffect, useRef, createRef, forwardRef, useLayoutEffect } from 'react';
...
function QuizBuildQuestions({ focusProp, quizQuestions, setQuizQuestions }) {
  ...
  const [explanations, setExplanations] = useState(quizQuestions.map(() => ''));
  ...
  function addNewQuestion() {
    ...
    for (const choice of quizQuestions[lastIndexQuizQuestions]?.choices || []) {
      ...
      if (singleChoice.trim().length === 0) {
        toast.error(`Please ensure that all previous choices are filled out!`);
        return;
      }
    }

    if (quizQuestions[lastIndexQuizQuestions]?.correctAnswer.length === 0) {
      toast.error(`Please ensure to fill out the correct answer!`);
      return;
    }
  ...
  function deleteQuestion(singleQuestion) {
    const filteredQuestions = quizQuestions.filter(question => singleQuestion.id !== question.id);
    const updatedRefs = textAreaRefs.current.filter((_, index) => quizQuestions[index].id !== singleQuestion.id);
  ...
  function handleInputChange(index, text) {
    const updatedQuestions = quizQuestions.map((question, i) => (
      index === i ? { ...question, mainQuestion: text } : question
    ));
    ...
    const updatedQuestions = quizQuestions.map((question, i) => {
      if (questionIndex === i) {
        const updatedChoices = question.choices.map((choice, j) => (
          choiceIndex === j ? `${prefixes[j]}. ${text}` : choice
        ));
  ...
  function updateCorrectAnswer(text, questionIndex) {
    ...
    updatedQuestions[questionIndex].correctAnswer = correctAnswersArray.indexOf(text);
    setQuizQuestions(updatedQuestions);
  }
  ...
  function handleExplanationChange(index, text) {
    const updatedQuestions = quizQuestions.map((question, i) => (
      index === i ? { ...question, explanation: text } : question
    ));
    setQuizQuestions(updatedQuestions);
  }
    ...
    <div className="p-3 mt-6 flex justify-between border border-green-700 rounded-md relative">
      ...
      <div className="flex gap-2 flex-col w-full">
        ...
        <textarea
          ...
          className="border border-gray-200 rounded-md p-3 w-full h-[70px] resize-none text-[13px] outline-none"
        />
      </div>
          </div>
        ))}
