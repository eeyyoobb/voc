...
const Users = () => {
  ...
  const { mutate: mutateUpdateUser, isLoading: isLoadingUpdateUser } =
    useMutation({
      mutationFn: ({ isAdmin,isEditor, userId }) => {
        return updateProfile({
          token: userState.userInfo.token,
          userData: { admin: isAdmin, editor: isEditor},
          userId,
        });
      },
      onSuccess: (data) => {
  ...
  const handleAdminCheck = (event, userId) => {
    ...
    if (
      ...
      mutateUpdateUser({ isAdmin: event.target.checked, isEditor: event.target.checked, userId , });
    } else {
      event.target.checked = initialCheckValue;
    }
  };

  const handleEditorCheck = (event, userId) => {
    const initialCheckValue = !event.target.checked;
  
    if (window.confirm("Do you want to change the editor status of this user?")) {
      mutateUpdateUser({ isAdmin: event.target.checked, isEditor: event.target.checked, userId });
    } else {
      event.target.checked = initialCheckValue;
    }
      ...
      {usersData?.data.map((user) => (
        <tr key={user._id}>
          ...
          <input
                ...
                onChange={(event) => handleEditorCheck(event, user._id)}
                disabled={isLoadingUpdateUser}
              />
